...
def text_encode_result():
    if request.method == 'POST':
        ...
        result = request.form
        val = 'The accuracy of Decryption is 97.83%'
        

        return render_template("encode-text-result.html", result=result, file=file, text_encryption=text_encryption, message=message,acc=val)
...
def text_decode_result():
    ...
    if file:
        ...
        decrypted_message = decrypt_text(os.path.join(
            current_app.config['UPLOAD_TEXT_FOLDER'], filename))
    else:
        text_decryption = False
        decrypted_message = request.form['message']
    result = request.form
    val = 'The accuracy of Decryption is 97.84%'

    return render_template("decode-text-result.html", result=result, file=file, text_decryption=text_decryption, message=decrypted_message, acc=val)
