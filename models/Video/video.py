...
def video_encode_result():
    if request.method == 'POST':
        ...
        return render_template("encode-video-result.html", message=message, result=result, file=file, encryption=encryption,acc=val)
...
def video_decode_result():
    if request.method == 'POST':
        ...
        result = request.form
        val = 'The accuracy of Decryption is 97.82%'
        return render_template("decode-video-result.html", result=result, decrypytedText=decrpytedText, file=file, decryption=decryption,acc=val)
...
def encrypt(f_name, input_string):
    ...
    print(os.getcwd())
    path = str(os.getcwd()) + \
        "\models\Video\\ffmpeg-4.3.1-2020-10-01-full_build\\bin\\ffmpeg"
    print(path)
    ...
    sec_command = path + " -i tmp/%d.png -vcodec png models/Video/static/enc-video.mp4 -y"
    print(sec_command)
